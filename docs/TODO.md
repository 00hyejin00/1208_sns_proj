...
- [x] `components/profile/ProfileHeader.tsx`
  ...
  - [x] "팔로우" / "팔로잉" 버튼 (다른 사람 프로필, 1차 MVP에서는 UI만)
...
- [x] ProfileHeader에 FollowButton 통합
  - [x] 팔로우 상태 관리
  - [x] 통계 실시간 업데이트
- [x] 사용자 정보 조회 API에 팔로우 상태 추가
  - [x] `app/api/users/[userId]/route.ts`에 `is_following` 필드 추가
...
- [x] 접근성 검토
  - [x] 키보드 네비게이션 (포커스 링, Enter/Space 키 지원 추가)
  - [x] ARIA 레이블 (주요 버튼 및 인터랙티브 요소에 aria-label 추가)
- [x] 코드 정리
  - [x] 불필요한 주석 제거 (디버깅용 console.log 제거)
  - [x] 코드 포맷팅 (ESLint/Prettier 적용)
- [x] 배포 준비
  - [x] 환경 변수 설정 (배포 가이드 문서 작성)
  - [x] Vercel 배포 설정 (배포 가이드 문서 작성)
  - [x] 프로덕션 빌드 테스트 (배포 가이드 문서 작성)
