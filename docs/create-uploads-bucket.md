...
게시물 이미지 업로드를 위해 `uploads` Storage 버킷을 생성해야 합니다.
...
4. **New bucket** 버튼 클릭
5. 버킷 정보 입력:
   - **Name**: `uploads` (정확히 이 이름으로 입력)
   - **Public bucket**: ❌ 체크 해제 (비공개 버킷)
   - **File size limit**: `6291456` (6MB) 또는 원하는 크기
...
- 버킷 이름은 정확히 `uploads`여야 합니다 (대소문자 구분)
- Service Role 클라이언트를 사용하므로 RLS 정책이 필요 없습니다
- 버킷이 비공개(private)로 설정되어 있어도 Service Role로 접근 가능합니다
